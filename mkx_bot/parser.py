"""
Регулярки + функции для извлечения всех полей из сообщения канала.

Оригинальный формат (пример из ТЗ):
...
# "#ХищникГоро" — сводное имя матча (без пробелов)
RE_COMBINED_TAG = re.compile(r"^\s*#([^\s#]+)\s*$", re.MULTILINE)

# "#Хищник #Горо" — раздельные русские имена (именно 2 тега подряд)
RE_SPLIT_TAGS = re.compile(
    r"^\s*#(?P<p1>[^\s#]+)\s+#(?P<p2>[^\s#]+)\s*$",
    re.MULTILINE,
)

# "Predator - Goro" (только английские символы и пробелы в именах)
...
# Метка завершения матча "#T8"
RE_MATCH_END = re.compile(r"#T(?P<n>\d+)\b")


def _first(pattern: re.Pattern, text: str) -> Optional[re.Match]:
    return pattern.search(text)


def parse_message(text: str, message_id: Optional[int] = None) -> Optional[ParsedMessage]:
    """Главная функция. Возвращает ParsedMessage или None, если это не
    сообщение формата @statamk10."""
    if not text:
        return None
    ...
    # Локальное время матча (наивное) — для фильтров/коридоров
    match_time = None
    match_ts = None
    try:
        ...
        # timestamp здесь как «наивный» — интерпретация даётся в аналайзере.
        match_ts = int(dt.timestamp())
    except Exception:
        pass

    # Разделенные русские имена
    p1_ru = p2_ru = None
    for m in RE_SPLIT_TAGS.finditer(text):
        # Пропускаем строку с коэффициентами («P1/P2 - ...» содержит #)
        line = m.group(0)
    ...
    # Английские имена — первая строка вида "Foo - Bar"
    p1_en = p2_en = None
    for m in RE_EN_NAMES.finditer(text):
        cand1, cand2 = m.group("p1").strip(), m.group("p2").strip()
        if cand1.lower() in ("p1m", "p1", "fyes", "fno", "timestat"):
            continue
        # Отбрасываем строки, в которых встречаются ключевые слова
        # коэффициентов — они не могут быть именами.
        line = m.group(0)
